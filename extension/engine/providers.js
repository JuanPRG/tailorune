...
export const ROUTES = {
  ...
  // ling is RETIRED and the other two are both on the JSON exclusion list, so
  // this route contributes ZERO eligible entries to every task -- confirmed
  // against v4, which reports 0 openrouter entries for resume, letter and
  // judge alike. It stays because the chain does, and because deleting it
  // would hide that fact rather than record it.
  openrouter: {
...
export const TASK_CHAINS = {
  // LLM_RESUME_JSON_PROVIDER_CHAIN
  resume: ['gemini31_flash_lite', 'groq_qwen36', 'groq_oss120', 'cerebras_oss120', 'openrouter'],
  skills: ['gemini31_flash_lite', 'groq_qwen36', 'groq_oss120', 'cerebras_oss120', 'openrouter'],
